...
static void deparseIndexElem(StringInfo str, IndexElem* index_elem)
{
	...
	switch (index_elem->ordering)
	{
		...
		case SORTBY_USING:
			...
			Assert(false);
			break;
		/* YB-EXT: emit HASH for hash-distributed column ordering */
		case SORTBY_HASH:
...
static void deparseSortBy(StringInfo str, SortBy* sort_by)
{
	...
	switch (sort_by->sortby_dir)
	{
		...
		case SORTBY_USING:
			...
			break;
		/* YB-EXT: HASH not valid in ORDER BY but bison grammar accepts it;
		 * deparse as HASH for round-trip fidelity. Downstream YB rejects it. */
		case SORTBY_HASH:
			appendStringInfoString(str, "HASH ");
			break;
	}
