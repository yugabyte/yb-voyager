...
agents: dict[str, Agent] = {
    ...
    # "voyager-agent": Agent(
    #     description="A database migration assistant", graph=voyager_agent
    # ),
    "rag-assistant": Agent(
...
async def get_all_agent_info() -> list[AgentInfo]:
    if "voyager-agent" not in agents:
        from agents.voyager_agent import compile_voyager_agent
        voyager_agent = await compile_voyager_agent()
        agents["voyager-agent"] = Agent(description="A database migration assistant", graph=voyager_agent)
    
    print(f"\nAgents in get_all_agent_info: {agents}\n")
    return [
        AgentInfo(key=agent_id, description=agent.description) for agent_id, agent in agents.items()
    ]


def add_agent(agent_id: str, agent: CompiledStateGraph, description: str) :
    agents[agent_id] = Agent(description=description, graph=agent)