...
func init() {
	...
	// Multi-tenant composite key UNIQUE(tenant_id, slug) with shared tenants;
	// all slugs renamed to globally fresh values => zero conflicts under
	// composite-tuple semantics.
	// CANARY (flipped 2026-07-02): with exporters that write multi-column
	// unique-index metadata (a323249bb+), detection compares full tuples and
	// finds no conflicts. Artifacts generated by OLDER exporters carry
	// flattened per-column metadata and false-positive on tenant_id — if this
	// assertion fails with conflicts detected, regenerate the artifact with a
	// current yb-voyager binary (CDCBENCH_REGEN=1).
	Register(Workload{
