...
| 2 | Table | **Create new table** (no PK) | Export DB | Exp-Drift | Same as **#1** while the heap stays outside the capture set (**silent**). **`export data`** does **not** fail on restart merely because a new heap exists — the no-PK guardrail (**`reportUnsupportedTablesForLiveMigration`**) **`ErrExit`**s only when a **listed** export table has no PK (`finalTableList` ∩ non-PK heaps). **`ErrExit`** once the table is **included** for live CDC (restart flags, patterns, MSR/config edits, etc.) **without** a PK. | Add PK **before** including it in live export (then restart / separate migration as usual); otherwise exclude from live CDC | D |
