...
| 7 | Table | **Detach partition** | Export DB | Exp-Drift | Partition was in Debezium `table.include.list` and `SourceRenameTablesMap`. After detach it becomes a standalone table — rename mapping to root is now semantically wrong. Events may still arrive under the leaf name but get misrouted. | Restart migration | *To be tested* |
