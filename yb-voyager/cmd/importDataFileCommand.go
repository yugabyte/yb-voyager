...
func init() {
	...
	importDataFileCmd.Flags().Lookup("start-clean").Usage =
...
Note that for the cases where a table doesn't have a primary key, this may lead to insertion of duplicate data. To avoid this, exclude the table from the --file-table-map or truncate those tables manually before using the start-clean flag`
}
