...
func prepareForImportDataCmd(importFileTasks []*ImportFileTask) {
	...
	// Preparing data file descriptor to prepare import data file command
	...
	/*
		For import batch recovery mode, we need list of tables' columns to prepare the INSERT ON CONFLICT statement

		So IF
			hasHeader is false, we need to fetch the actual columns list from target for each table
		ELSE
			split the headers from the first line of the file and use it as column list
			This is already handled: prepareTableToColumns() in importData.go

		Prepopulating dataFileDescriptor for column list might be better way
			so that import data code is untouched for this
	*/
	var tableNameToExportedColumns = make(map[string][]string)
	if !hasHeader {
		yb, ok := tdb.(*tgtdb.TargetYugabyteDB)
		if !ok {
			utils.ErrExit("failed to cast target DB to YBTargetDB")
		}

		// fetch the column list from target DB
		for _, task := range importFileTasks {
			tableName := task.TableNameTup
			columns, err := yb.GetTableColumns(tableName)
			if err != nil {
				utils.ErrExit("failed to get columns for table %q: %v", tableName, err)
			}
			tableNameToExportedColumns[tableName.ForMinOutput()] = columns
		}
		log.Infof("table to columns mapping generated by quering target DB")
		dataFileDescriptor.TableNameToExportedColumns = tableNameToExportedColumns
	} else {
		log.Infof("table to columns mapping will be generated by reading header from the data file")
	}

	log.Infof("generated data file descriptor for import data file: %+v", dataFileDescriptor)
