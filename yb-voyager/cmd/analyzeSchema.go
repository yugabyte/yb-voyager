	...
	for _, column := range columns {
		/*
			e.g. 1. CREATE TABLE public.citext_type (
				...
				stmt:{create_stmt:{relation:{schemaname:"public"  relname:"citext_type"  inh:true  relpersistence:"p"  location:14}  table_elts:{column_def:{colname:"id"
				type_name:{names:{string:{sval:"pg_catalog"}}  names:{string:{sval:"int4"}}  typemod:-1  location:42}  is_local:true  location:39}}  table_elts:{column_def:{colname:"data"
				type_name:{names:{string:{sval:"public"}}  names:{string:{sval:"citext"}}  typemod:-1  location:60}  is_local:true  location:55}}  oncommit:ONCOMMIT_NOOP}}  stmt_len:75
		...
		*/
		if column.GetColumnDef() != nil {
			...
			if slices.Contains(unsupportedIndexDatatypes, typeNameOnIdx0) || slices.Contains(unsupportedIndexDatatypes, typeNameOnIdx1) {
