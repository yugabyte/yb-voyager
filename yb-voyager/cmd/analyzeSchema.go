...
func reportCase(filePath string, reason string, ghIssue string, suggestion string, objType string, objName string, sqlStmt string, issueType string, docsLink string, impact string) {
	...
	for _, sqlInfo := range sqlInfoArr {
		if sqlInfo.objName != "" && !slices.Contains(supportedExtensionsOnYB, sqlInfo.objName) {
			...
			reportCase(fpath, UNSUPPORTED_EXTENSION_ISSUE+" Refer to the docs link for the more information on supported extensions.", "https://github.com/yugabyte/yb-voyager/issues/1538", "", "EXTENSION",
				sqlInfo.objName, sqlInfo.formattedStmt, UNSUPPORTED_FEATURES, EXTENSION_DOC_LINK, constants.IMPACT_LEVEL_3)
		}
...
func analyzeSchemaInternal(sourceDBConf *srcdb.Source, detectIssues bool) utils.SchemaReport {
	...
	*/
	...
	schemaAnalysisReport.MigrationComplexity = calculateMigrationComplexity(sourceDBConf.DBType, schemaDir, schemaAnalysisReport)
	...
	for _, sqlStmtInfo := range sqlInfoArr {
		...
		if ok {
			...
			reportCase(filePath, CONVERSION_ISSUE_REASON, "https://github.com/yugabyte/yugabyte-db/issues/10866",
				"Remove it from the exported schema", "CONVERSION", convName, sqlStmtInfo.formattedStmt, UNSUPPORTED_FEATURES, CREATE_CONVERSION_DOC_LINK, constants.IMPACT_LEVEL_3)
		} else {
