...
For source type is PG -
	we are sure that all the sequences being migrated are attached to the tables so we can skip the sequence if it is not attached to a
...
func restoreSequencesInOfflineMigration(msr *metadb.MigrationStatusRecord, importTableList []sqlname.NameTuple) error {
	...
	sequenceLastValue, err := fetchSequenceLastValueMap(isDebeziumExport, sourceType)
	if err != nil {
	...
	if shouldFilterSequences(sourceType) {
		...
		sequenceNameToTableMap, err := fetchSequenceToTableListMap(sourceColumnToSequenceMapping)
		if err != nil {
			return fmt.Errorf("failed to fetch sequence to table list map: %w", err)
		}
		sequenceNameTupleToLastValueMap, err = filterSequencesAsPerImportTableList(sequenceLastValue, importTableList, sequenceNameToTableMap)
		if err != nil {
...
func fetchSequenceLastValueMap(isDebeziumExport bool, sourceType string) (map[string]int64, error) {
...
func fetchSequenceToTableListMap(sourceColumnToSequenceMapping map[string]string) (*utils.StructMap[sqlname.NameTuple, []string], error) {
...
func checkIfSequenceAttachedToTablesInTableList(importTableList []sqlname.NameTuple, sequenceAttachedTables []string) (bool, error) {
	for _, table := range sequenceAttachedTables {
...
func filterSequencesAsPerImportTableList(sequenceLastValue map[string]int64, importTableList []sqlname.NameTuple, sequenceNameToTableMap *utils.StructMap[sqlname.NameTuple, []string]) (*utils.StructMap[sqlname.NameTuple, int64], error) {
	sequenceNameTupleToLastValueMap := utils.NewStructMap[sqlname.NameTuple, int64]()
	...
	for sequenceName, lastValue := range sequenceLastValue {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to check if sequence is attached to tables in table list: %w", err)
		}
		...
		if !sequenceTuple.TargetTableAvailable() {
			...
			//return error
			return nil, fmt.Errorf("sequence %q is not present in the target database", sequenceName)
		}
		//restore the sequence last value
		sequenceNameTupleToLastValueMap.Put(sequenceTuple, lastValue)
