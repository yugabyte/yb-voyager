...
func importData(importFileTasks []*ImportFileTask) {
	...
	defer restoreGeneratedByDefaultAsIdentityColumns(importFileTasks)
...
func restoreGeneratedByDefaultAsIdentityColumns(importFileTasks []*ImportFileTask) {
	tables := importFileTasksToTableNames(importFileTasks)
	log.Infof("restoring generated by default as identity columns for tables: %v", tables)
	tablesToIdentityColumnNames := getGeneratedByDefaultAsIdentityColumnsNamesForTables(tables)
...
func getGeneratedByDefaultAsIdentityColumnsNamesForTables(tables []string) map[string][]string {
	var result = make(map[string][]string)
	var identityType = "BY DEFAULT"
