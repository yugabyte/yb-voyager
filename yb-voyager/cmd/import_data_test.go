...
func TestExportAndImportDataSnapshotReport_ErrorPolicyStashAndContinue_OnPkConflictIgnore(t *testing.T) {
	ctx := context.Background()
	...
	// Create table in the default public schema in Postgres and YugabyteDB.
	createTableSQL := `CREATE TABLE public.test_data_unique (id INTEGER PRIMARY KEY, name TEXT, email TEXT UNIQUE);`
	postgresContainer.ExecuteSqls(createTableSQL)
	testYugabyteDBTarget.TestContainer.ExecuteSqls(createTableSQL)
	t.Cleanup(func() {
		postgresContainer.ExecuteSqls("DROP TABLE IF EXISTS public.test_data_unique;")
		testYugabyteDBTarget.TestContainer.ExecuteSqls("DROP TABLE IF EXISTS public.test_data_unique;")
	})

	// Insert data into Postgres.
	for i := 1; i <= 100; i++ {
		stmt := fmt.Sprintf("INSERT INTO public.test_data_unique (id, name, email) VALUES (%d, 'name_%d', 'name_%d@example.com');", i, i, i)
		postgresContainer.ExecuteSqls(stmt)
	}

	// Insert conflicting rows into YugabyteDB to simulate errors.
	for i := 1; i <= 10; i++ {
		differentI := i * 10000 // Ensure different IDs to avoid primary key conflict
		// same i for email to create unique constraint violation
		stmt := fmt.Sprintf("INSERT INTO public.test_data_unique (id, name, email) VALUES (%d, 'name_%d', 'name_%d@example.com');", differentI, differentI, i)
		testYugabyteDBTarget.TestContainer.ExecuteSqls(stmt)
	}
	...
	// Ensure the snapshotRowsMap contains the expected data.
	tblName := sqlname.NameTuple{
		SourceName:  sqlname.NewObjectNameWithQualifiedName(POSTGRESQL, "public", "public.test_data_unique"),
		CurrentName: sqlname.NewObjectNameWithQualifiedName(POSTGRESQL, "public", "public.test_data_unique"),
	}
	rowCountPair, _ := snapshotRowsMap.Get(tblName)
	assert.Equal(t, int64(90), rowCountPair.Imported, "Imported row count mismatch")
	assert.Equal(t, int64(10), rowCountPair.Errored, "Errored row count mismatch")
	...
	assert.Equal(t, &tableMigStatusOutputRow{
		TableName:          `public."test_data_unique"`,
		FileName:           "",
		ImportedCount:      90,
		ErroredCount:       10,
		TotalCount:         100,
	...
	// Verify that the backup directory contains the expected error files.
	// error file is expected to be under dir table::test_data/file::test_data_data.sql:1960b25c and of the name ingestion-error.batch::1.10.10.92.E
	tableDir := fmt.Sprintf("table::%s", "test_data_unique")
	fileDir := fmt.Sprintf("file::test_data_unique_data.sql:%s", importdata.ComputePathHash(filepath.Join(tempExportDir, "data", "test_data_unique_data.sql")))
	tableFileErrorsDir := filepath.Join(backupDir, "data", "errors", tableDir, fileDir)
	errorFilePath := filepath.Join(tableFileErrorsDir, "ingestion-error.batch::1.10.10.283.E")
	assert.FileExistsf(t, errorFilePath, "Expected error file %s to exist", errorFilePath)

	// Verify the content of the error file
	testutils.AssertFileContains(t, errorFilePath, `duplicate key value violates unique constraint "test_data_unique_email_key"`)
	testutils.AssertFileContains(t, errorFilePath, PARTIAL_BATCH_ERROR_NOTE)
}
