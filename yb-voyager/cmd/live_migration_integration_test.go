...
func TestLiveMigrationWithUniqueKeyConflictDetectionCases(t *testing.T) {
	ctx := context.Background()
...
END $$;`,
	...
	ok = utils.RetryWorkWithTimeout(5, 200, func() bool {
		return streamingPhaseCompleted(t, postgresContainer.GetConfig().Password,
			yugabytedbContainer.GetConfig().Password, 2000, 4000, 1000, `test_schema."test_live"`)
	})
	assert.True(t, ok)
