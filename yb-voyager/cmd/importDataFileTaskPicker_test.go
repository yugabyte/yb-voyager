...
func TestSequentialTaskPickerResumePicksInProgressTask(t *testing.T) {
	...
	fbp, err := NewFileBatchProducer(task1, state, false, errorHandler, progressReporter)
	testutils.FatalIfError(t, err)
...
func TestColocatedAwareRandomTaskPickerResumable(t *testing.T) {
	...
	assert.NoError(t, err)
	fbp1, err := NewFileBatchProducer(task1, state, false, errorHandler, progressReporter)
	...
	assert.NoError(t, err)
	fbp2, err := NewFileBatchProducer(task2, state, false, errorHandler, progressReporter)
	...
	assert.NoError(t, err)
	fbp3, err := NewFileBatchProducer(task3, state, false, errorHandler, progressReporter)
...
func TestColocatedCappedRandomTaskPickeResumable(t *testing.T) {
	...
	assert.NoError(t, err)
	fbp4, err := NewFileBatchProducer(task4, state, false, errorHandler, progressReporter)
