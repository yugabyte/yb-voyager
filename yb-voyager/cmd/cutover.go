...
func cutoverInitiatedAlready(exporterOrImporterRole string) (bool, error) {
	if !dbzm.IsMigrationInStreamingMode(exportDir) {
		return false, nil
	}
	...
	switch exporterOrImporterRole {
	case SOURCE_DB_EXPORTER_ROLE:
		if msr.CutoverToTargetRequested {
			return true, nil
		}
	case TARGET_DB_EXPORTER_FB_ROLE:
		if msr.CutoverToSourceRequested {
			return true, nil
		}
	case TARGET_DB_EXPORTER_FF_ROLE:
		if msr.CutoverToSourceReplicaRequested {
			return true, nil
		}
	case TARGET_DB_IMPORTER_ROLE:
		if msr.CutoverToTargetRequested {
	...
	default:
		panic(fmt.Sprintf("invalid role %s", exporterOrImporterRole))
	}
	return false, nil
}
