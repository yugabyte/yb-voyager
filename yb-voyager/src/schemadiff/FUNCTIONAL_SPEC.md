...
**Corresponding design spec:** [Design & Architecture](ARCHITECTURE.md)
...
- **Performance:** Snapshot capture uses read-only `pg_catalog` queries; diffing is in-memory. Background drift checks run on a separate goroutine and must not block CDC/export/import processing.
- **Scale:** Cost grows with schema metadata size (tables, columns, indexes, etc.).
- **Operability:** Terminal-first output; optional HTML/JSON reports.
- **Reliability:** Hooks degrade gracefully on snapshot/compare failures; exit drift check is best-effort.

### Limitations and restrictions

Detailed user-facing caveats are listed under **§3.2 Surface area → Caveats and unsupported features**.

- Oracle and MySQL sources are out of scope for catalog-based comparison in this release.
- No automatic remediation; users apply DDL manually.
- C3 requires ignore rules for expected PG-vs-YB differences.

---

## 3 Surface Area & Usage

### Default behavior

- `**yb-voyager schema diff`:** Opt-in — run the command when you want a report; it is not invoked automatically by other phases (unless the user runs it, e.g. after a drift warning).
- **Live migration (`export data` / `import data`):** Proactive drift detection runs with those phases: pre-start snapshot gate (continue/abort prompt when drift is found), periodic background checks (default **10 minutes**), and a **best-effort** drift check on process exit. This spec does not define a user flag to disable drift detection (implementation may add one later).

Which comparison runs where: see the **Summary: hooks and default command behavior** block under **Comparison model** in §3.2.

---

### Surface area

#### Comparison model (C1, C2, C3)

Three comparisons power the entire feature. Each has a specific purpose.

#### C1 — Latest source snapshot vs live source
...
####  Latest target snapshot vs live target
...
#### Summary: hooks and default command behavior
...
#### Command: `yb-voyager schema diff`
...
#### Command output: terminal summary
...
#### Command output: full saved report
...
#### Proactive drift detection during live migration
...
1. If the user selects `N`, the command exits. If `y`, export proceeds with the
  new snapshot as the reference for periodic checks.
2. If no differences are found, export proceeds normally. A log line confirms:
  `No source schema changes detected since last snapshot.`

##### Pre-start gate — `import data`

Same logic as the export-data pre-start gate but for the target side:
...
1. `N` exits. `y` proceeds with the new snapshot as the periodic check
  reference.

##### Periodic drift checks during `export data` (C1)
...
##### Periodic drift checks during `import data` (C2)

Same structure as periodic export-data checks, comparing live target against the latest saved target
snapshot.
...
| `export data` starts                                | Source pre-export (S2)           | Compared against S1 as a pre-start gate (see pre-start gate — `export data` above). Periodic drift checks compare against this. |
...
| `import data` starts                                | Target pre-import (T2)           | Compared against T1 as a pre-start gate (see pre-start gate — `import data` above). Periodic drift checks compare against this. |
...
can reference them via `--list-snapshots` and `--compare` (see **Command: `yb-voyager schema diff`** above).
...
#### Suppressed differences (C3 ignore rules)
...
##### PG-vs-YB expected differences (always suppressed in C3)
...
##### Objects pending `finalize-schema-post-data-import`
...
Illustrative invocations (see also **Command output** examples under §3.2).
...
Schema drift detection is additive. It does not change migration data paths; it reads catalogs and compares snapshots.


| Voyager capability                        | Interaction                                                                                                                 |
| ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| Assess migration                          | N/A — not modified.                                                                                                         |
| Export schema                             | Writes source baseline snapshot (S1); enables C1 and default `schema diff` sections.                                        |
| Analyze schema                            | N/A — not modified.                                                                                                         |
| Export data                               | C1 pre-start gate, periodic checks, exit check; writes further source snapshots.                                            |
| Import schema                             | Writes target baseline snapshot (T1).                                                                                       |
| Import data                               | C2 pre-start gate, periodic checks, exit check; writes further target snapshots.                                            |
| Import data to source / source-replica    | N/A — not in scope for this feature.                                                                                        |
| End migration                             | N/A — no change.                                                                                                            |
| Live migration (fall forward / back)      | Hooks apply during `export data` / `import data` as today; no change to fall-forward semantics beyond new prompts/warnings. |
| Offline migration (with/without Debezium) | `schema diff` usable anytime; background hooks are live-migration-only.                                                     |
| Status commands                           | N/A — not modified.                                                                                                         |
| SSL                                       | Uses same connection configuration as other commands (flags / config file).                                                 |
| Case sensitivity / reserved words         | Same as existing Voyager PG/YB behavior.                                                                                    |
| Multiple schemas                          | Supported via `--source-db-schema` / target equivalent in config.                                                           |
| Connectivity                              | Requires live DB access for comparisons that use `live-source` or `live-target`.                                            |
| File import                               | N/A — not modified.                                                                                                         |
| Docker                                    | No new container requirements; integration tests may use testcontainers.                                                    |
| Resumption                                | Pre-start gate compares new snapshot to previous on each resume / start clean.                                              |
| Sizing                                    | Negligible extra disk for JSON snapshots under `{export-dir}/metainfo/schema/snapshots/`.                                   |
| Airgapped installer                       | No new network dependencies for the feature itself.                                                                         |
| Callhome                                  | N/A — no change specified.                                                                                                  |
| YugabyteDB UI                             | N/A — not integrated.                                                                                                       |


### Impact on supported databases


| Database   | Role   | Support in this feature                       |
| ---------- | ------ | --------------------------------------------- |
| PostgreSQL | Source | **Supported** for catalog snapshot and C1/C3. |
| YugabyteDB | Target | **Supported** for catalog snapshot and C2/C3. |
| Oracle     | Source | **Deferred** — not in this release.           |
| MySQL      | Source | **Deferred** — not in this release.           |


### Versioning and upgrades

Upgrading `yb-voyager` mid-migration: users without historical snapshots still get **C3** (live vs live). After upgrade, new snapshots are written at the next hook point. Ignore rules may be **YB version-aware** (see [ARCHITECTURE.md](ARCHITECTURE.md)). No change to export-dir layout beyond new snapshot files.

---

## 5 Noteworthy Considerations
...
| Airgapped installation | Yes            | No external network calls for this feature.    |
...
|                          | Yes / No / N/A | Notes                                                                    |
| ------------------------ | -------------- | ------------------------------------------------------------------------ |
| New perf testing planned | No             | Catalog queries are lightweight; background drift check is non-blocking. |
...
|                   | Yes / No / N/A | Notes                                                                          |
| ----------------- | -------------- | ------------------------------------------------------------------------------ |
| Offline migration | N/A            | `schema diff` anytime; hooks only during live migration.                       |
| Live migration    | Yes            | Hooks during `export data` / `import data`.                                    |
| SSL               | Yes            | Inherits SSL from connection flags / config.                                   |
| New tests planned | Yes            | Unit tests (diff, ignore rules, severity); integration (testcontainers PG+YB). |
| Jenkins pipeline  | Yes            | For integration suite.                                                         |
| Sizing impact     | N/A            | Snapshot JSON files small vs migration data.                                   |


### Impact to existing reports (supportability)


|                     | Yes / No / N/A | Notes                                                |
| ------------------- | -------------- | ---------------------------------------------------- |
| Assess report       | N/A            | Unchanged.                                           |
| Analyze report      | N/A            | Unchanged.                                           |
| Command line output | Yes            | New `schema diff`; drift warnings in live migration. |


### Documentation changes

- **New:** `yb-voyager schema diff`, snapshot lifecycle, drift warnings during live migration, C1/C2/C3 semantics.
- **Caveats:** Facts-only diff output; no auto-fix; PG-only source in v1; fixed 10-minute periodic interval; best-effort exit check.

### Impact (resource consumption)

- **CPU:** Periodic snapshot + diff comparable to short catalog query bursts; background goroutine only.
- **Memory:** Holds in-memory snapshot structures during compare; bounded by schema size.
- **Disk:** One JSON file per snapshot under `{export-dir}/metainfo/schema/snapshots/`; optional HTML/JSON reports under `{export-dir}/reports/`.
...
| ---------------------------------------- | ----------------------------------------------------------------------------------- |
| [Design & Architecture](ARCHITECTURE.md) | Detailed design: catalog queries, object coverage, ignore rules, integration paths. |
| QA Test Spec                             | TBD                                                                                 |
| Performance Test Plan                    | TBD                                                                                 |
