...
// TestLiveMigrationMultiLevelPartitioning tests live migration with multi-level partitioned tables
// where the root table has no primary key but leaf partitions do.
//
// Schema:
//   - sales: root table partitioned by LIST on year, NO PRIMARY KEY
//   - sales_2023: intermediate partition for year 2023, further partitioned by LIST on region
//   - sales_2024: intermediate partition for year 2024, further partitioned by LIST on region
//   - sales_2023_us: leaf partition for 2023 US with PRIMARY KEY (id)
//   - sales_2023_eu: leaf partition for 2023 EU with PRIMARY KEY (id)
//   - sales_2024_us: leaf partition for 2024 US with PRIMARY KEY (id)
//   - sales_2024_eu: leaf partition for 2024 EU with PRIMARY KEY (id)
//
// Hierarchy:
//
//	sales (root, no PK)
//	├── sales_2023 (intermediate, no PK)
//	│   ├── sales_2023_us (leaf, has PK)
//	│   └── sales_2023_eu (leaf, has PK)
//	└── sales_2024 (intermediate, no PK)
//	    ├── sales_2024_us (leaf, has PK)
//	    └── sales_2024_eu (leaf, has PK)
func TestLiveMigrationMultiLevelPartitioning(t *testing.T) {
	t.Parallel()
	lm := NewLiveMigrationTest(t, &TestConfig{
		SourceDB: ContainerConfig{
			Type:         "postgresql",
			ForLive:      true,
			DatabaseName: "test_multilevel",
		},
		TargetDB: ContainerConfig{
			Type:         "yugabytedb",
			DatabaseName: "test_multilevel",
		...
		SchemaNames: []string{"public"},
		SchemaSQL: []string{
			// Create root partitioned table with NO PRIMARY KEY
			...
			// Create intermediate partitions (also partitioned, no PK)
			`CREATE TABLE public.sales_2023 PARTITION OF public.sales 
				FOR VALUES IN (2023) PARTITION BY LIST (region);`,
			`CREATE TABLE public.sales_2024 PARTITION OF public.sales 
				FOR VALUES IN (2024) PARTITION BY LIST (region);`,

			// Create leaf partitions WITH primary keys
			`CREATE TABLE public.sales_2023_us PARTITION OF public.sales_2023 FOR VALUES IN ('US');`,
			`ALTER TABLE public.sales_2023_us ADD PRIMARY KEY (id);`,

			`CREATE TABLE public.sales_2023_eu PARTITION OF public.sales_2023 FOR VALUES IN ('EU');`,
			`ALTER TABLE public.sales_2023_eu ADD PRIMARY KEY (id);`,

			`CREATE TABLE public.sales_2024_us PARTITION OF public.sales_2024 FOR VALUES IN ('US');`,
			`ALTER TABLE public.sales_2024_us ADD PRIMARY KEY (id);`,

			`CREATE TABLE public.sales_2024_eu PARTITION OF public.sales_2024 FOR VALUES IN ('EU');`,
			`ALTER TABLE public.sales_2024_eu ADD PRIMARY KEY (id);`,
		},
		SourceSetupSchemaSQL: []string{
			// Set replica identity for CDC on root and all partitions
			`ALTER TABLE public.sales REPLICA IDENTITY FULL;`,
			`ALTER TABLE public.sales_2023 REPLICA IDENTITY FULL;`,
			`ALTER TABLE public.sales_2024 REPLICA IDENTITY FULL;`,
			`ALTER TABLE public.sales_2023_us REPLICA IDENTITY FULL;`,
			`ALTER TABLE public.sales_2023_eu REPLICA IDENTITY FULL;`,
			`ALTER TABLE public.sales_2024_us REPLICA IDENTITY FULL;`,
			`ALTER TABLE public.sales_2024_eu REPLICA IDENTITY FULL;`,
		},
		InitialDataSQL: []string{
			// Insert initial data into different leaf partitions
			`INSERT INTO public.sales (id, year, region, amount) VALUES (1, 2023, 'US', 100.00);`,
			`INSERT INTO public.sales (id, year, region, amount) VALUES (2, 2023, 'US', 200.00);`,
			`INSERT INTO public.sales (id, year, region, amount) VALUES (3, 2023, 'EU', 150.00);`,
			`INSERT INTO public.sales (id, year, region, amount) VALUES (4, 2024, 'US', 250.00);`,
			`INSERT INTO public.sales (id, year, region, amount) VALUES (5, 2024, 'EU', 300.00);`,
			`INSERT INTO public.sales (id, year, region, amount) VALUES (6, 2024, 'EU', 350.00);`,
		},
		SourceDeltaSQL: []string{
			// CDC operations across different leaf partitions
			`INSERT INTO public.sales (id, year, region, amount) VALUES (7, 2023, 'US', 400.00);`,
			`INSERT INTO public.sales (id, year, region, amount) VALUES (8, 2024, 'EU', 500.00);`,
			`UPDATE public.sales SET amount = 999.99 WHERE id = 1;`,
			`DELETE FROM public.sales WHERE id = 5;`,
		},
		CleanupSQL: []string{
	...
	// Wait for snapshot to complete - 6 initial rows tracked by root table
	err = lm.WaitForSnapshotComplete(map[string]int64{
		`"public"."sales"`: 6,
	}, 60*time.Second)
	testutils.FatalIfError(t, err, "failed to wait for snapshot complete")
	...
	err = lm.WithSourceTargetConn(func(source, target *sql.DB) error {
		leafPartitions := []string{
			...
			"public.sales_2023_eu",
			"public.sales_2024_us",
	...
	// Wait for streaming to complete
	// SourceDeltaSQL: insert 2, update 1, delete 1
	err = lm.WaitForForwardStreamingComplete(map[string]ChangesCount{
		`"public"."sales"`: {
			Inserts: 2,
			Updates: 1,
	...
	err = lm.WithSourceTargetConn(func(source, target *sql.DB) error {
		var sourceCount, targetCount int
		err := source.QueryRow("SELECT COUNT(*) FROM public.sales").Scan(&sourceCount)
		if err != nil {
			return fmt.Errorf("count source sales: %w", err)
		}
		err = target.QueryRow("SELECT COUNT(*) FROM public.sales").Scan(&targetCount)
		if err != nil {
			return fmt.Errorf("count target sales: %w", err)
		}
		if sourceCount != targetCount {
			return fmt.Errorf("total row count mismatch: source=%d, target=%d", sourceCount, targetCount)
		}
...
// TestLiveMigrationMultiSchemaPartitioning tests live migration with partitioned tables
// where partitions span multiple schemas. The root table has no primary key but leaf
// partitions do.
//
// Schema:
//   - public.transactions: root table in public schema, NO PRIMARY KEY
//   - public.transactions_domestic: partition in public schema with PRIMARY KEY (id)
//   - intl.transactions_international: partition in intl schema with PRIMARY KEY (id)
//   - archive.transactions_archive: partition in archive schema with PRIMARY KEY (id)
//
// This tests that CDC events with partitions in different schemas are handled correctly.
func TestLiveMigrationMultiSchemaPartitioning(t *testing.T) {
	t.Parallel()
	lm := NewLiveMigrationTest(t, &TestConfig{
		SourceDB: ContainerConfig{
			Type:         "postgresql",
			ForLive:      true,
			DatabaseName: "test_multischema",
		},
		TargetDB: ContainerConfig{
			Type:         "yugabytedb",
			DatabaseName: "test_multischema",
		},
		SchemaNames: []string{"public", "intl", "archive"},
		SchemaSQL: []string{
			// Create additional schemas
			`CREATE SCHEMA IF NOT EXISTS intl;`,
			`CREATE SCHEMA IF NOT EXISTS archive;`,

			// Create root partitioned table in public schema with NO PRIMARY KEY
			...
			// Create partition in public schema
			`CREATE TABLE public.transactions_domestic 
				PARTITION OF public.transactions FOR VALUES IN ('domestic');`,
			`ALTER TABLE public.transactions_domestic ADD PRIMARY KEY (id);`,

			// Create partition in intl schema
			`CREATE TABLE intl.transactions_international 
				PARTITION OF public.transactions FOR VALUES IN ('international');`,
			`ALTER TABLE intl.transactions_international ADD PRIMARY KEY (id);`,

			// Create partition in archive schema
			`CREATE TABLE archive.transactions_archive 
				PARTITION OF public.transactions FOR VALUES IN ('archive');`,
			`ALTER TABLE archive.transactions_archive ADD PRIMARY KEY (id);`,
		},
		SourceSetupSchemaSQL: []string{
			// Set replica identity for CDC on root and all partitions
			`ALTER TABLE public.transactions REPLICA IDENTITY FULL;`,
			`ALTER TABLE public.transactions_domestic REPLICA IDENTITY FULL;`,
			`ALTER TABLE intl.transactions_international REPLICA IDENTITY FULL;`,
			`ALTER TABLE archive.transactions_archive REPLICA IDENTITY FULL;`,
		},
		InitialDataSQL: []string{
			// Insert initial data into different partitions across schemas
			`INSERT INTO public.transactions (id, txn_type, amount, description) 
				VALUES (1, 'domestic', 100.00, 'Local purchase');`,
			`INSERT INTO public.transactions (id, txn_type, amount, description) 
				VALUES (2, 'domestic', 200.00, 'Another local purchase');`,
			`INSERT INTO public.transactions (id, txn_type, amount, description) 
				VALUES (3, 'international', 500.00, 'Foreign exchange');`,
			`INSERT INTO public.transactions (id, txn_type, amount, description) 
				VALUES (4, 'international', 750.00, 'Import payment');`,
			`INSERT INTO public.transactions (id, txn_type, amount, description) 
				VALUES (5, 'archive', 50.00, 'Old transaction');`,
		},
		SourceDeltaSQL: []string{
			// CDC operations across different schema partitions
			`INSERT INTO public.transactions (id, txn_type, amount, description) 
				VALUES (6, 'domestic', 300.00, 'New local');`,
			`INSERT INTO public.transactions (id, txn_type, amount, description) 
				VALUES (7, 'international', 1000.00, 'Big international');`,
			`INSERT INTO public.transactions (id, txn_type, amount, description) 
				VALUES (8, 'archive', 25.00, 'Another archive');`,
			`UPDATE public.transactions SET amount = 999.99, description = 'Updated' WHERE id = 1;`,
			`DELETE FROM public.transactions WHERE id = 5;`,
		},
		CleanupSQL: []string{
			`DROP TABLE IF EXISTS public.transactions CASCADE;`,
			`DROP SCHEMA IF EXISTS intl CASCADE;`,
	...
	err = lm.StartExportData(true, map[string]string{
		"--use-partition-root=false": "",
		"--table-list":               "public.transactions",
	})
	testutils.FatalIfError(t, err, "failed to start export data")
	...
	// Wait for snapshot to complete - 5 initial rows tracked by root table
	err = lm.WaitForSnapshotComplete(map[string]int64{
	...
	err = lm.WithSourceTargetConn(func(source, target *sql.DB) error {
		var sourceCount, targetCount int
		err := source.QueryRow("SELECT COUNT(*) FROM public.transactions").Scan(&sourceCount)
		if err != nil {
			return fmt.Errorf("count source transactions: %w", err)
		}
		err = target.QueryRow("SELECT COUNT(*) FROM public.transactions").Scan(&targetCount)
		if err != nil {
			return fmt.Errorf("count target transactions: %w", err)
		}
		if sourceCount != targetCount {
			return fmt.Errorf("total row count mismatch: source=%d, target=%d", sourceCount, targetCount)
		}
