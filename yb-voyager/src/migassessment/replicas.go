...
// ReplicaDiscoveryInfo containing discovery details and validated replicas.
// Returns empty ReplicaDiscoveryInfo if no replicas (single-node assessment) or if source is not PostgreSQL.
func HandleReplicaDiscoveryAndValidation(source *srcdb.Source, replicaEndpointsFlag string) (ReplicaDiscoveryInfo, error) {
