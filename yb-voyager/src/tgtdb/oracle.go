...
END;`, BATCH_METADATA_TABLE_SCHEMA, BATCH_METADATA_TABLE_SCHEMA)
	...
	tableWiseEventsMetadataTableQuery := fmt.Sprintf(`BEGIN
	EXECUTE IMMEDIATE 'CREATE TABLE %s (
		migration_uuid VARCHAR2(36),
		table_name VARCHAR2(250),
		channel_no INT,
...
func (tdb *TargetOracleDB) GetGeneratedAlwaysIdentityColumnNamesForTable(table string) ([]string, error) {
	...
	err := tdb.WithConn(func(conn *sql.Conn) (bool, error) {
		...
		for rows.Next() {
			...
			if err != nil {
				log.Errorf("error in scanning identity column for table %s: %v", table, err)
				return false, fmt.Errorf("error in scanning identity column for table %s: %w", table, err)
			}
...
func (tdb *TargetOracleDB) GetGeneratedAlwaysAsIdentityColumnsForTables(tables []string) (map[string][]string, error) {
	var result = make(map[string][]string)
	for _, table := range tables {
		identityColumns, err := tdb.GetGeneratedAlwaysIdentityColumnNamesForTable(table)
		if err != nil {
...
func (tdb *TargetOracleDB) DisableGeneratedAlwaysAsIdentityColumns(tableColumnsMap map[string][]string) error {
	...
	for table, columns := range tableColumnsMap {
		...
		for _, column := range columns {
			...
			query := fmt.Sprintf(`ALTER TABLE %s MODIFY %s GENERATED BY DEFAULT AS IDENTITY (START WITH LIMIT VALUE)`, qualifiedTblName, column)
			err := tdb.WithConn(func(conn *sql.Conn) (bool, error) {
				_, err := conn.ExecContext(context.Background(), query)
				if err != nil {
					log.Errorf("error executing query-%s to alter always identity column(%s) for table(%s): %v", query, column, qualifiedTblName, err)
					return false, fmt.Errorf("error executing query-%s to alter always identity column(%s) for table(%s): %w", query, column, qualifiedTblName, err)
				}
				return false, nil
			})
			if err != nil {
...
func (tdb *TargetOracleDB) EnableGeneratedAlwaysAsIdentityColumns(tableColumnsMap map[string][]string) error {
	...
	for table, columns := range tableColumnsMap {
		...
		for _, column := range columns {
			// LIMIT VALUE - ensures that start it is set to the current value of the sequence
			query := fmt.Sprintf(`ALTER TABLE %s MODIFY %s GENERATED ALWAYS AS IDENTITY(START WITH LIMIT VALUE)`, qualifiedTblName, column)
			err := tdb.WithConn(func(conn *sql.Conn) (bool, error) {
				_, err := conn.ExecContext(context.Background(), query)
				if err != nil {
					log.Errorf("error executing query-%s to alter default identity column(%s) for table(%s): %v", query, column, qualifiedTblName, err)
					return false, fmt.Errorf("error executing query-%s to alter default identity column(%s) for table(%s): %w", query, column, qualifiedTblName, err)
				}
				return false, nil
			})
			if err != nil {
