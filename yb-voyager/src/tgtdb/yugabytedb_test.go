...
func TestGetGeneratedStoredColumns(t *testing.T) {
	...
	_, err := yb.Exec(`CREATE SCHEMA test_generated_stored`)
	require.NoError(t, err)
	defer func() { _, _ = yb.Exec(`DROP SCHEMA IF EXISTS test_generated_stored CASCADE`) }()

	_, err = yb.Exec(`CREATE TABLE test_generated_stored.probe (
			id INT,
			val INT GENERATED ALWAYS AS (id + 1) STORED
		)`)
	if err != nil {
		t.Skipf("YugabyteDB does not support stored generated columns: %v", err)
	}
	...
	plain := testutils.CreateNameTupleWithTargetName("test_generated_stored.plain", "public", YUGABYTEDB)
	identityOnly := testutils.CreateNameTupleWithTargetName("test_generated_stored.identity_only", "public", YUGABYTEDB)
	withGenerated := testutils.CreateNameTupleWithTargetName("test_generated_stored.with_generated", "public", YUGABYTEDB)
	mixedCase := testutils.CreateNameTupleWithTargetName("test_generated_stored.\"MixedCase\"", "public", YUGABYTEDB)
	twoGenerated := testutils.CreateNameTupleWithTargetName("test_generated_stored.two_generated", "public", YUGABYTEDB)
	probe := testutils.CreateNameTupleWithTargetName("test_generated_stored.probe", "public", YUGABYTEDB)
	ukOnGenerated := testutils.CreateNameTupleWithTargetName("test_generated_stored.uk_on_generated", "public", YUGABYTEDB)
	ukIndexOnGenerated := testutils.CreateNameTupleWithTargetName("test_generated_stored.uk_index_on_generated", "public", YUGABYTEDB)

	tableList := []sqlname.NameTuple{plain, identityOnly, withGenerated, mixedCase, twoGenerated, probe, ukOnGenerated, ukIndexOnGenerated}

	_, err = yb.Exec(`CREATE TABLE test_generated_stored.partitioned (
			id INT,
			data TEXT,
			qty INT,
			amount INT GENERATED ALWAYS AS (qty + 1) STORED
		) PARTITION BY LIST (data)`)
	var partitioned sqlname.NameTuple
	hasPartitioned := false
	if err == nil {
		_, err = yb.Exec(`CREATE TABLE test_generated_stored.partitioned_l PARTITION OF test_generated_stored.partitioned FOR VALUES IN ('London')`)
		require.NoError(t, err)
		_, err = yb.Exec(`CREATE TABLE test_generated_stored.partitioned_s PARTITION OF test_generated_stored.partitioned FOR VALUES IN ('Sydney')`)
		require.NoError(t, err)
		partitioned = testutils.CreateNameTupleWithTargetName("test_generated_stored.partitioned", "public", YUGABYTEDB)
		tableList = append(tableList, partitioned)
		hasPartitioned = true
	} else {
		t.Logf("skipping partitioned generated-column case: %v", err)
	}

	empty, err := yb.GetGeneratedStoredColumns(nil, true)
	require.NoError(t, err)
	assert.Empty(t, empty.Keys())

	got, err := yb.GetGeneratedStoredColumns(tableList, true)
	require.NoError(t, err)

	_, ok = got.Get(plain)
	assert.False(t, ok, "plain table should not appear")
	_, ok = got.Get(identityOnly)
	assert.False(t, ok, "identity-only table should not appear")

	assertGeneratedStoredColumns(t, got, probe, []GeneratedStoredColumn{{Name: "val", InUniqueIndex: false}})
	assertGeneratedStoredColumns(t, got, withGenerated, []GeneratedStoredColumn{{Name: "amount", InUniqueIndex: false}})
	assertGeneratedStoredColumns(t, got, mixedCase, []GeneratedStoredColumn{{Name: "Area", InUniqueIndex: false}})
	assertGeneratedStoredColumns(t, got, twoGenerated, []GeneratedStoredColumn{
		{Name: "sum_ab", InUniqueIndex: false},
		{Name: "prod_ab", InUniqueIndex: false},
	})
	assertGeneratedStoredColumns(t, got, ukOnGenerated, []GeneratedStoredColumn{{Name: "amount", InUniqueIndex: true}})
	assertGeneratedStoredColumns(t, got, ukIndexOnGenerated, []GeneratedStoredColumn{{Name: "amount", InUniqueIndex: true}})
	if hasPartitioned {
		assertGeneratedStoredColumns(t, got, partitioned, []GeneratedStoredColumn{{Name: "amount", InUniqueIndex: false}})
	}
}

func assertGeneratedStoredColumns(t *testing.T, got *utils.StructMap[sqlname.NameTuple, []GeneratedStoredColumn], table sqlname.NameTuple, expected []GeneratedStoredColumn) {
	t.Helper()
