...
func (yb *TargetYugabyteDB) GetGeneratedAlwaysIdentityColumnNamesForTable(table string) ([]string, error) {
	...
	err := yb.connPool.WithConn(func(conn *pgx.Conn) (bool, error) {
		...
		if err != nil {
			log.Errorf("error in querying identity columns for table %s: %v", table, err)
			return false, fmt.Errorf("error in querying identity columns for table %s: %w", table, err)
		}
		defer rows.Close()
		for rows.Next() {
			var colName string
			err = rows.Scan(&colName)
			if err != nil {
				log.Errorf("error in scanning identity column name: %v", err)
				return false, fmt.Errorf("error in scanning identity column name: %w", err)
			}
...
func (yb *TargetYugabyteDB) GetGeneratedAlwaysAsIdentityColumnsForTables(tables []string) (map[string][]string, error) {
	var result = make(map[string][]string)
	for _, table := range tables {
		identityColumns, err := yb.GetGeneratedAlwaysIdentityColumnNamesForTable(table)
		if err != nil {
			log.Errorf("error in getting identity columns for table %s: %v", table, err)
			return nil, fmt.Errorf("error in getting identity columns for table %s: %w", table, err)
		}
...
func (yb *TargetYugabyteDB) DisableGeneratedAlwaysAsIdentityColumns(tableColumnsMap map[string][]string) error {
	...
	for table, columns := range tableColumnsMap {
		qualifiedTblName := yb.qualifyTableName(table)
		batch := pgx.Batch{}
		for _, column := range columns {
			query := fmt.Sprintf(`ALTER TABLE %s ALTER COLUMN %s SET GENERATED BY DEFAULT`, qualifiedTblName, column)
			batch.Queue(query)
		}
		err := yb.connPool.WithConn(func(conn *pgx.Conn) (bool, error) {
			br := conn.SendBatch(context.Background(), &batch)
			for i := 0; i < batch.Len(); i++ {
				_, err := br.Exec()
				if err != nil {
					log.Errorf("error executing query to alter always identity columns for table(%s): %v", qualifiedTblName, err)
					return false, fmt.Errorf("error executing query to alter always identity columns for table(%s): %w", qualifiedTblName, err)
				}
			}
			if err := br.Close(); err != nil {
				log.Errorf("error closing batch of queries to alter alwayss identity columns for table(%s): %v", qualifiedTblName, err)
				return false, fmt.Errorf("error closing batch of queries to alter always identity columns for table(%s): %w", qualifiedTblName, err)
			}
			return false, nil
		})
		if err != nil {
			log.Errorf("error in disabling identity columns for table %s: %v", qualifiedTblName, err)
			return fmt.Errorf("error in disabling identity columns for table %s: %w", qualifiedTblName, err)
		}
	}
	return nil
}
...
func (yb *TargetYugabyteDB) EnableGeneratedAlwaysAsIdentityColumns(tableColumnsMap map[string][]string) error {
	...
	for table, columns := range tableColumnsMap {
		...
		batch := pgx.Batch{}
		for _, column := range columns {
			query := fmt.Sprintf(`ALTER TABLE %s ALTER COLUMN %s SET GENERATED ALWAYS`, qualifiedTblName, column)
			batch.Queue(query)
		}
		err := yb.connPool.WithConn(func(conn *pgx.Conn) (bool, error) {
			br := conn.SendBatch(context.Background(), &batch)
			for i := 0; i < batch.Len(); i++ {
				_, err := br.Exec()
				if err != nil {
					log.Errorf("error executing query to alter default identity columns for table(%s): %v", qualifiedTblName, err)
					return false, fmt.Errorf("error executing query to alter default identity columns for table(%s): %w", qualifiedTblName, err)
				}
			}
			if err := br.Close(); err != nil {
				log.Errorf("error closing batch of queries to alter default identity columns for table(%s): %v", qualifiedTblName, err)
				return false, fmt.Errorf("error closing batch of queries to alter default identity columns for table(%s): %w", qualifiedTblName, err)
			}
			return false, nil
		})
		if err != nil {
			log.Errorf("error in enabling identity columns for table %s: %v", qualifiedTblName, err)
			return fmt.Errorf("error in enabling identity columns for table %s: %w", qualifiedTblName, err)
		}
	}
	return nil
}
