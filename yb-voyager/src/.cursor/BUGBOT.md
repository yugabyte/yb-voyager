...
- `srcdb` — Source DB interface and implementations (PostgreSQL, Oracle, MySQL, YugabyteDB-as-source). Schema extraction, data export queries, permission checks, replication setup.
- `tgtdb` — Target DB interface and implementations (YugabyteDB, PostgreSQL). COPY/import logic, conflict detection, parallelism, partition resolution.
- `metadb` — SQLite metadata DB for migration state (MSR), import file status, and queue segments.
- `namereg` — Name registry mapping table/sequence names across source and target across exporter/importer roles.
- `query/queryissue` — Detects DDL/DML compatibility issues and generates fix recommendations for assessment reports.
...
- `importdata` — Import error handling, error policies, and Prometheus metrics.
...
- `adaptiveparallelism` — Adjusts import parallelism dynamically based on target cluster load.
