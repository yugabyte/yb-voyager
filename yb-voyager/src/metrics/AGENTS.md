...
- `yb_voyager_import_data_cdc_conflicts_total` (counter) — streaming CDC events that had to block on a detected unique-key conflict, incremented once per blocked event (not per conflicting cached event or per re-wait). Labels: `+ importer_role, table_name`. importer_role is the commad from which metrics is collected and table name is a qualified table name. Only produced on the PG→YB import path where conflict detection runs (Oracle sources use `PARTITION_BY_TABLE` and never conflict).
