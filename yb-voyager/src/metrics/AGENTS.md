...
- `yb_voyager_import_data_cdc_conflicts_total` (counter) — streaming CDC events that had to block on a detected unique-key conflict, incremented once per blocked event (not per conflicting cached event or per re-wait). Labels: `+ importer_role, table_name`. The `table_name` value is **anonymized** (schema+table run through the shared `anon` anonymizer, precomputed per table at conflict-cache init) so no raw identifier reaches the scrape endpoint — unlike the snapshot metrics' raw `table_name`. Only produced on the PG→YB import path where conflict detection runs (Oracle sources use `PARTITION_BY_TABLE` and never conflict).
