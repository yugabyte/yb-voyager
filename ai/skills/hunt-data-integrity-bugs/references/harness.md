...
- Oracle after the last phase: wait for quiescence (poll `ValidateDataConsistency` until it passes twice ~15s apart, the importer stops, or a timeout), latching `stopped` the first time `IsStopped()` returns true. Then record `stopped`, the `ValidateDataConsistency` error, per-partition counts when relevant, and counts of `ERROR`/`FATAL`/`unexpected rows affected` lines in `<export-dir>/logs/yb-voyager-import-data.log`.
