...
YugabyteDB Voyager (`yb-voyager`) is a database migration CLI tool (Go) with a Debezium CDC plugin (Java/Maven). It supports offline and live migrations from PostgreSQL, Oracle, and MySQL to YugabyteDB. See `README.md` for end-user details.
...
- **Go 1.24.2** — required by `go.mod`. On Linux CI: installed at `/usr/local/go/bin/go`.
- **JDK 17** (not 21+) — the installer script enforces Java 17–19 for the Debezium build. On Linux CI: installed at `/usr/lib/jvm/java-17-openjdk-amd64`. Set `JAVA_HOME` accordingly.
...
- **YugabyteDB** — target database. On Linux CI: installed at `/opt/yugabyte-2025.2.1.0`.
- **`rsync`** — required by the installer's `-l` (local build) flag.

Set these env vars before building or running:
```
export JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64   # adjust path for your OS
...
- The YugabyteDB data directory must be owned by the running user (not root) — `sudo chown -R ubuntu:ubuntu /opt/yugabyte-*` if you hit permission errors on start.
