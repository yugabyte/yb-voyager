        ...
        (id, amount, branch, region) 
            SELECT 
                ...
                region[1 + mod(n, array_length(region, 1))] 
                    FROM amount_list, region_list, generate_series(1,1000000) as n;
...
INSERT INTO emp SELECT num, 'user_' || num , (RANDOM()*50)::INTEGER FROM generate_series(1,1000000) AS num;
...
WITH status_list AS (
        ...
        (id, statuses, arr)
            SELECT  n,
                    ...
                    (RANDOM()*200)::INTEGER
                        FROM generate_series(1,1000000) AS n, status_list;
