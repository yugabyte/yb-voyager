...
CREATE TABLE composite_type_examples.ordinary_table (
    ...
    _basic composite_type_examples.basic_comp_type,
    nested composite_type_examples.nested,
    _nested composite_type_examples.nested,
...
CREATE OR REPLACE FUNCTION composite_type_examples.populate_generated_columns()
RETURNS TRIGGER AS $$
BEGIN
    NEW._basic := NEW.basic_;
    NEW._nested := NEW.nested;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER populate_generated_columns_trigger
BEFORE INSERT OR UPDATE ON composite_type_examples.ordinary_table
FOR EACH ROW
EXECUTE FUNCTION composite_type_examples.populate_generated_columns();
...
CREATE TRIGGER populate_generated_columns_trigger
BEFORE INSERT OR UPDATE ON composite_type_examples.inherited_table
FOR EACH ROW
EXECUTE FUNCTION composite_type_examples.populate_generated_columns();
...
CREATE TABLE enum_example.bugs (
    ...
    _status enum_example.bug_status,
    severity enum_example.bug_severity,
    _severity enum_example.bug_severity,
    info enum_example.bug_info
);

CREATE OR REPLACE FUNCTION enum_example.populate_generated_columns()
RETURNS TRIGGER AS $$
BEGIN
    NEW._status := NEW.status;
    NEW._severity := NEW.severity;
    NEW.info := enum_example.make_bug_info(NEW.status, NEW.severity);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER populate_generated_columns_trigger
BEFORE INSERT OR UPDATE ON enum_example.bugs
FOR EACH ROW
EXECUTE FUNCTION enum_example.populate_generated_columns();
...
CREATE TRIGGER populate_generated_columns_trigger
BEFORE INSERT OR UPDATE ON enum_example.bugs_clone
FOR EACH ROW
EXECUTE FUNCTION enum_example.populate_generated_columns();
...
-- type "base_type_examples.int42" is only a shell
