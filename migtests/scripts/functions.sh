...
grant_user_permission_postgresql() {
	...
	db_schema=$2
	conn_string="postgresql://${SOURCE_DB_ADMIN_USER}:${SOURCE_DB_ADMIN_PASSWORD}@${SOURCE_DB_HOST}:${SOURCE_DB_PORT}/${db_name}"
	psql "${conn_string}" -v db_user="${SOURCE_DB_USER}" -v schema_list="${db_schema}" -v replication_group='replication_group' -v original_owner_of_tables="${SOURCE_DB_ADMIN_USER}" -v db_name="${db_name}" -v is_live_migration=0 -v is_live_migration_fall_back=0 -f ../../../../guardrails-scripts/yb-voyager-pg-grant-migration-permissions.sql
...
grant_permissions_for_live_migration_pg() {
	...
	db_schema=$2
	conn_string="postgresql://${SOURCE_DB_ADMIN_USER}:${SOURCE_DB_ADMIN_PASSWORD}@${SOURCE_DB_HOST}:${SOURCE_DB_PORT}/${db_name}"
	psql "${conn_string}" -v db_user="${SOURCE_DB_USER}" -v schema_list="${db_schema}" -v replication_group='replication_group' -v original_owner_of_tables="${SOURCE_DB_ADMIN_USER}" -v db_name="${db_name}" -v is_live_migration=1 -v is_live_migration_fall_back=0 -f ../../../../guardrails-scripts/yb-voyager-pg-grant-migration-permissions.sql
}
