#!/usr/bin/env python3
"""
callhome_schema_prep.py — turn the Voyager callhome anonymized DDL into a
...
Usage:  python3 callhome_schema_prep.py in.sql [--schema public] > out.sql   (report on stderr)
"""
import re
import sys
...
def prep(text, to_schema=None):
    ...
    if add:
        out += ["", "-- callhome_schema_prep: synthetic PKs for base tables missing one"] + add
...
def main():
    ...
    if "--schema" in args:
        ...
        del args[i:i + 2]
    if len(args) != 1:
        sys.exit("usage: callhome_schema_prep.py in.sql [--schema NAME] > out.sql")
