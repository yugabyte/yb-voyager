...
def verify_colocation(tgt, source_db_type):
    ...
    for table in sharded_tables:
        ...
        assert (
            not actual_colocation
        ), f"Table '{table_name}' colocation mismatch. Expected: False, Actual: {actual_colocation}"
    ...
    for table in colocated_tables:
        ...
        assert (
            actual_colocation
        ), f"Table '{table_name}' colocation mismatch. Expected: True, Actual: {actual_colocation}"
...
class PostgresDB:
    ...
    def get_sum_of_column_of_table(
        self, table_name, column_name, schema_name="public"
    ) -> int:
        cur = self.conn.cursor()
    ...
    def get_count_index_on_table(self, schema_name="public") -> Dict[str, int]:
        cur = self.conn.cursor()
        cur.execute(
            f"SELECT tablename, count(indexname) FROM pg_indexes WHERE schemaname = '{schema_name}' GROUP  BY tablename;"
        )
        return {tablename: cnt for tablename, cnt in cur.fetchall()}

    def get_distinct_values__of_column_of_table(
        self, table_name, column_name, schema_name="public"
    ) -> List[Any]:
        cur = self.conn.cursor()
    ...
    def get_functions_count(self, schema_name="public") -> int:
        cur = self.conn.cursor()
        cur.execute(
            f"SELECT count(routine_name) FROM  information_schema.routines WHERE  routine_type = 'FUNCTION' AND routine_schema = '{schema_name}' AND routine_name NOT IN ({self.EXPECTED_ORAFCE_FUNCTIONS});"
        )
        return cur.fetchone()[0]
    ...
    def count_sequences(self, schema_name="public") -> int:
        cur = self.conn.cursor()
        cur.execute(
            f"select count(sequence_name) from information_schema.sequences where sequence_schema='{schema_name}';"
        )
        return cur.fetchone()[0]

    def get_column_to_data_type_mapping(
        self, schema_name="public"
    ) -> Dict[str, Dict[str, str]]:
        cur = self.conn.cursor()
        cur.execute(
            f"SELECT table_name, column_name, data_type FROM information_schema.columns WHERE table_schema = '{schema_name}' and table_name NOT IN ({self.EXPECTED_ORAFCE_VIEWS})"
        )
        tables = {}
    ...
    def invalid_index_present(self, table_name, schema_name="public"):
        cur = self.conn.cursor()
        cur.execute(
            f"select indisvalid from pg_index where indrelid = '{schema_name}.{table_name}'::regclass::oid"
        )
    ...
    def fetch_all_triggers(self, schema_name="public") -> List[str]:
        cur = self.conn.cursor()
        cur.execute(
            f"SELECT trigger_name FROM information_schema.triggers WHERE trigger_schema = '{schema_name}'"
        )
    ...
    def fetch_all_procedures(self, schema_name="public") -> List[str]:
        cur = self.conn.cursor()
        cur.execute(
            f"SELECT routine_name FROM information_schema.routines WHERE routine_schema = '{schema_name}'AND routine_name NOT IN ({self.EXPECTED_ORAFCE_FUNCTIONS}); "
        )
    ...
    def fetch_partitions(self, table_name, schema_name) -> int:
        cur = self.conn.cursor()
        cur.execute(
            f"SELECT count(*) AS partitions FROM pg_catalog.pg_inherits WHERE inhparent = '{schema_name}.{table_name}'::regclass;"
        )
        return cur.fetchone()[0]
    ...
    def fetch_all_schemas(self) -> set[str]:
        cur = self.conn.cursor()
        cur.execute(
            f"SELECT schema_name FROM information_schema.schemata where schema_name !~ '^pg_' and schema_name <> 'information_schema' and schema_name NOT IN ({self.EXPECTED_ORAFCE_SCHEMAS})"
        )
        return set(cur.fetchall())

    def get_identity_type_columns(
        self, type_name, table_name, schema_name="public"
    ) -> List[str]:
        cur = self.conn.cursor()
        cur.execute(
            f"SELECT column_name FROM information_schema.columns WHERE table_schema = '{schema_name}' AND table_name = '{table_name}' AND is_identity='YES' AND identity_generation='{type_name}'"
        )
        return [column[0] for column in cur.fetchall()]

    def assert_distinct_values_of_col(
        self,
        table_name,
        column_name,
        schema_name="public",
        ...
        distinct_values = self.get_distinct_values__of_column_of_table(
            table_name, column_name, schema_name
        )
        for distinct_value in distinct_values:
            if transform_func:
                transformed_distinct_value = (
                    transform_func(distinct_value) if distinct_value else distinct_value
                )
            else:
    ...
    def assert_all_values_of_col(
        ...
        if transform_func:
            all_values = [
                transform_func(value) if value else value for value in all_values
            ]
    ...
    def get_text_length(
        self, primary_key, id, column, table_name, schema_name="public"
    ) -> int:
        cur = self.conn.cursor()
        cur.execute(
            f"SELECT length({column}) FROM {schema_name}.{table_name} WHERE {primary_key} = {id}"
        )
        return cur.fetchone()[0]
    ...
    def check_table_colocation(self, table_name, schema_name="public") -> bool:
        cur = self.conn.cursor()
        cur.execute(
            f'SELECT is_colocated FROM yb_table_properties(\'"{schema_name}"."{table_name}"\'::regclass);'
        )
        return cur.fetchone()[0]
