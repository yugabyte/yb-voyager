...
# Set DEBEZIUM_MAX_HEAP environment variable to the required RAM size for debezium to use. Default is 3GB. Added this to avoid OOM issues with large row tests.
exec "$JAVA_BINARY" $DEBEZIUM_OPTS $JAVA_OPTS -Xmx${DEBEZIUM_MAX_HEAP:-3g} $DEBUGGER -cp "$RUNNER"$PATH_SEP"conf"$PATH_SEP$LIB_PATH$PATH_SEP$YB_OR_PG_CONNECTOR_PATH -Dquarkus.config.locations=$PROPERTIES_FILE_PATH -Doracle.net.tns_admin=$TNS_ADMIN io.debezium.server.Main
